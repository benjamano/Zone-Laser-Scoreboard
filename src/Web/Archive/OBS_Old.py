...
class OBS:
    ...
    """
    def __init__(self, secrets, app, context):
        try:
            self.IP = secrets["OBSSERVERIP"]
            self.app = app
            self._context = context
            self.PORT = secrets["OBSSERVERPORT"]
            self.PASSWORD = secrets["OBSSERVERPASSWORD"]
            self.AvailableMonitor = ""
            self.AvailableMonitors = []
            self.Secrets = secrets
            self.obs = obs.ReqClient(host=self.IP, port=self.PORT, password=self.PASSWORD, timeout=3)
    ...
    def showSleepMode(self) -> bool:
        try:
            ...
            """
            try:
                with open(f"Data/display/OBSText.txt", "w") as f:
    ...
    def resetConnection(self) -> bool:
        try:
            ...
            try:
                os.startfile(f"Utilities/appShortcuts/OBS.lnk", arguments='--disable-shutdown-check')
            except Exception as e:
                return False
