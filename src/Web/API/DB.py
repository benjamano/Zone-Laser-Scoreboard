        ...
        try:
            with self.app.app_context():
                if not Permission.query.first():
                    self.Insert(Permission(name="Admin", isActive=True))
                    self.SaveChanges()
                ...
                if not Gun.query.first() and not Player.query.first():
                    ...
                    self.Insert(Gun(name="Alpha", defaultColor="Red"))
                    self.Insert(Gun(name="Apollo", defaultColor="Red"))
                    self.Insert(Gun(name="Chaos", defaultColor="Red"))
                    self.Insert(Gun(name="Cipher", defaultColor="Red"))
                    self.Insert(Gun(name="Cobra", defaultColor="Red"))
                    self.Insert(Gun(name="Comet", defaultColor="Red"))
                    self.Insert(Gun(name="Commander", defaultColor="Red"))
                    self.Insert(Gun(name="Cyborg", defaultColor="Red"))
                    self.Insert(Gun(name="Cyclone", defaultColor="Red"))
                    self.Insert(Gun(name="Delta", defaultColor="Red"))
                    self.Insert(Gun(name="Dodger", defaultColor="Green"))
                    self.Insert(Gun(name="Dragon", defaultColor="Green"))
                    self.Insert(Gun(name="Eagle", defaultColor="Green"))
                    self.Insert(Gun(name="Eliminator", defaultColor="Green"))
                    self.Insert(Gun(name="Elite", defaultColor="Green"))
                    self.Insert(Gun(name="Falcon", defaultColor="Green"))
                    self.Insert(Gun(name="Ghost", defaultColor="Green"))
                    self.Insert(Gun(name="Gladiator", defaultColor="Green"))
                    self.Insert(Gun(name="Hawk", defaultColor="Green"))
                    self.Insert(Gun(name="Hyper", defaultColor="Green"))
                    self.Insert(Gun(name="Inferno", defaultColor="Green"))

                    self.Insert(SystemControls(name="isInitialised", value=0))
                    
                    with self.app.app_context():
                        for fixture_name, channels in self.fixtureProfiles.items():
                            fixture = Fixture(name=fixture_name, noOfchannels=len(channels))
                            self.db.session.add(fixture)
                            self.db.session.flush()  

                            for index, (channelName, values) in enumerate(channels.items()):
                                channel = FixtureChannel(fixtureID=fixture.id, channelNo=index + 1, name=channelName)
                                    ...
                                    for valueName, valueRange in values.items():
                                        if isinstance(valueRange, list):
                                            for val in valueRange:
                                                value_entry = FixtureChannelValue(channelID=channel.id, value=val, name=valueName)
                                                self.db.session.add(value_entry)
                                        else:
                                            value_entry = FixtureChannelValue(channelID=channel.id, value=valueRange, name=valueName)
                                            self.db.session.add(value_entry)
                                elif isinstance(values, list):
                                    for val in values:
                                        value_entry = FixtureChannelValue(channelID=channel.id, value=val, name=str(val))
                                        self.db.session.add(value_entry)
