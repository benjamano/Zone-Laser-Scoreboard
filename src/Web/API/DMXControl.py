...
class dmx:
    ...
    def unPatchFixture(self, fixtureId):
        try:
            with self.app.app_context():
                patchedFixture : PatchedFixture = self._context.db.session.query(PatchedFixture).filter_by(id=fixtureId).first()
                if patchedFixture:
    ...
    def registerDimmerFixture(self, displayName):
        try:
            ...
            with self.app.app_context():
                patchedFixture : PatchedFixture = self._context.db.session.query(PatchedFixture).filter_by(fixtureId=fixtureTypeId, dmxControllerFixtureId=(fixture.id if fixture != None else 0)).first()
    ...
    def registerFixtureUsingType(self, displayName, fixtureType, startChannel):
        try:
            if int(fixtureType) in self.fixtureProfiles:
                ...
                with self.app.app_context():
                    patchedFixture : PatchedFixture = self._context.db.session.query(PatchedFixture).filter_by(fixtureId=fixtureType, dmxControllerFixtureId=(fixture.id if fixture != None else 0)).first()
    ...
    def registerFixtureUsingTypeId(self, displayName, fixtureTypeId, startChannel) -> int:
        try:
            with self.app.app_context():
                ...
                with self.app.app_context():
                    patchedFixture = self._context.db.session.query(PatchedFixture).filter_by(fixtureId=fixtureTypeId, fixtureName=displayName, dmxControllerFixtureId=(fixture.id if fixture != None else 0)).first()
    ...
    def getFixtures(self):
        ...
        try:
            with self.app.app_context():
                registeredFixtures : list[PatchedFixture] = self._context.db.session.query(PatchedFixture).order_by(PatchedFixture.dmxStartAddress).all()
                
                for registeredFixture in registeredFixtures:            
                    fixture : Fixture | None = self._context.db.session.query(Fixture).filter_by(id = registeredFixture.fixtureId).first()
